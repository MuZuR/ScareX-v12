...
exports.run = async (client, message, args) => {
    if(!args[0]) {
        ...
        embed.setDescription("<a:bot_hayir_2:737702534340149299> Hata | Örnek Kullanım= "+ ayarlar.prefix + `döviz USD \n Kur Birim Kodları= \`\`USD EUR AUD DKK GBP CHF SEK CAD KWD NOK JPY SAR BGN RON RUB IRR CNY PKR QAR\`\``);
    ...
    if (args[0] === "USD"){
        ...
        embed.addField(`Birim Kodu`,res.kod);
        message.channel.send({embed: embed});
    }
   if(args[0] === "EUR"){
...
message.channel.send({embed: embed});
  ...
  console.log(`döviz komutu ${message.author.username} Tarafından ${message.guild.name}Adlı sunucuda kullanıldı`)
}
